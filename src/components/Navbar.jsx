  ...
  return (
    <nav className={cn("fixed w-full z-40  transition-all duration-300 ", isScrolled?"py-3 bg-background/80 backdrop-blur-md shadow-xs":"py-5")}>
      ...
      <div className='container flex items-center justify-around'> 
        ...
        <a href="#profile" className='text-xl font-bold text-primary flex items-center'>
          <span className='relative z-10'>
            <span className='text-glow text-foreground'>Geetha Naga Durga Lahari Maddu</span> Portfolio
          </span>
        </a>
        ...
        <div className={cn("fixed inset-0 bg-background/95 backdroup-blur-md z-40 flex flex-col items-center justify-center",
          ...
          <div className='flex  flex-col space-y-8  text-xl'>
            {navItems.map((items,key)=>(
              <a key={key} href={items.href} className='text-foreground/80 hover:text-primary transition-colors duration-300' onClick={()=>setIsMenuOpen(false)}>{items.name}</a>
